...
import { ArrowLeft, MapPin, Receipt, ArrowRight, ImageIcon, TrendingUp, Users, Wallet, Pencil } from 'lucide-react';
        ...
        {isAdmin && (
          <div className="flex items-center gap-2 shrink-0">
            <Link href={`/transactions/${tx.id}/edit`}
              className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold text-[#0E3D40] bg-[#FAF5EA] hover:bg-[#F0E8D5] border border-[#E8DDC9] transition-all">
              <Pencil className="w-3.5 h-3.5" />
              Edit
            </Link>
            <DeleteButton txId={tx.id} txCode={txCode(tx.created_at)} />
          </div>
        )}
      </div>
